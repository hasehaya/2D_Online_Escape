...
namespace Photon.Realtime
{
    ...
    public class LoadBalancingPeer : PhotonPeer
    {
        ...
        [SuppressMessage("Domain reload", "UDR0001:Domain Reload Analyzer",
            Justification = "This just provides access to RegionHandler.PingImplementation which is taken care of.")]
 ...
 websocketType;    // the native socket plugin supports UDP as well
            }
#else
            ...
            bool usePropertiesKey = false)
        {
            ...
            if (roomOptions.MaxPlayers > 0)
            {
                ...
                byte maxPlayersAsByte =
                    roomOptions.MaxPlayers <= byte.MaxValue ? (byte)roomOptions.MaxPlayers : (byte)0;
        ...
        public virtual bool OpJoinRandomRoom(OpJoinRandomRoomParams opJoinRandomRoomParams)
        {
            ...
            if (opJoinRandomRoomParams.ExpectedMaxPlayers > 0)
            {
                // the following code is for compatibility with old and new servers. old use MaxPlayers, which has to be byte typed. MaxPlayersInt is available on new servers to allow int typed MaxPlayer values.
                // added to server 5.0.19.xyz / 6.0.19.xyz respectively
                byte maxPlayersAsByte = opJoinRandomRoomParams.ExpectedMaxPlayers <= byte.MaxValue
                    ? (byte)opJoinRandomRoomParams.ExpectedMaxPlayers
                    : (byte)0;
            ...
            opParameters[ParameterCode.AllowRepeats] =
                true; // enables temporary queueing for low ccu matchmaking situations
            ...
            string regionCode, EncryptionMode encryptionMode, ConnectionProtocol expectedProtocol)
        {
            ...
            if (encryptionMode == EncryptionMode.DatagramEncryptionGCM && expectedProtocol != ConnectionProtocol.Udp)
            {
                ...
                throw new NotSupportedException(
                    "Expected protocol set to UDP, due to encryption mode DatagramEncryptionGCM.");
            }
