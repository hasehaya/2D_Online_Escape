...
- プランターギミック（`GimmickPlanter`）
  - ブースターを4回使う想定の成長ギミック。
  - 1回目のブースターでルートを確定する（最初の1手でルート判定できる前提）。
  - 2回目で成長期に遷移し、`Leaf` 子オブジェクトの Image を表示して成長画像へ切り替える。
  - 4回目で成熟期に遷移し、成熟画像へ切り替わったあと収穫できる。
  - ルート判定に失敗した場合は枯れて初期状態へ戻る。
