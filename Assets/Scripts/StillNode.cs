...
public class StillNode : MonoBehaviour, IViewable
{
    ...
    [Tooltip("表示するスチル画像")] public Sprite stillImage;
    ...
    [Header("UI References")] [Tooltip("スチル画像を表示するImage (自動設定される場合は空でOK)")]
    public Image stillImageUI;
    ...
    {
        // スチル画像の表示処理
        if (stillImageUI != null && stillImage != null)
        {
            stillImageUI.sprite = stillImage;
            stillImageUI.gameObject.SetActive(true);
        }
    ...
    {
        // スチル画像を非表示にする処理などを実装
        if (stillImageUI != null)
        {
            stillImageUI.gameObject.SetActive(false);
        }
    }
